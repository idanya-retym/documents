...
\title{
    ...
    (4-Bit Binary + 3→7 Thermometric with Reorder Mux)
}
\author{Retym --- Analog/Mixed-Signal Design Group}
\date{Document Date: \today \\ Revision: 1.0}
...
\begin{itemize}
    ...
    \item \texttt{D[6:4]} --- 3-bit MSB segment, converted to 7-bit thermometric code.
\end{itemize}
...
\begin{verbatim}
                 ...
                 |                                                   |
 D[6:0] ------->|  +----------+    +------------------+             |
                 |  | Binary-to|    | Thermometric     |             |
         D[6:4]-+->| Thermo   +--->| Reorder Mux      +---> T[6:0] |
                 |  | Encoder  |    | (7x7)            |             |
                 |  +----------+    +------------------+             |
                 |                                                   |
         D[3:0]-+-----------------------------------------> B[3:0]  |
...
Converts 3-bit binary input \texttt{D[6:4]} to 7-bit thermometric output.
...
\begin{tabular}{@{}cc@{}}
\toprule
\textbf{D[6:4] (decimal)} & \textbf{Thermometric Output TH[6:0]} \\
\midrule
000 (0) & 0000000 \\
001 (1) & 0000001 \\
010 (2) & 0000011 \\
...
module dac_digital_top (
    input  wire [6:0]  dac_code,     // 7-bit DAC input
    ...
    // MSB binary-to-thermometric conversion
    bin2thermo u_enc (
        .bin    (dac_code[6:4]),
        .thermo (thermo_enc)
    );
...
\textbf{\#} & \textbf{Check} \\
\midrule
1 & Monotonicity: consecutive \texttt{dac\_code} increases active elements by exactly one. \\
2 & Full-scale: \texttt{dac\_code = 7'h7F} $\rightarrow$ all 4 binary + all 7 thermo active. \\
3 & Zero-scale: \texttt{dac\_code = 7'h00} $\rightarrow$ no elements active. \\
4 & Mux identity: default \texttt{mux\_cfg} produces \texttt{thermo\_out == thermo\_enc}. \\
5 & Mux permutation: all $7! = 5040$ valid permutations preserve output set. \\
6 & Invalid config: illegal \texttt{sel} values flagged by \texttt{cfg\_valid}. \\
7 & No glitches on \texttt{thermo\_out} when \texttt{mux\_cfg} is stable. \\
\bottomrule
\end{tabular}
